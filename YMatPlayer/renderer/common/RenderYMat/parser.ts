...
function getSourceBlob(type: number, sourceBody: ArrayBuffer | ArrayBufferView) {
  const body = sourceBody instanceof ArrayBuffer ? new Uint8Array(sourceBody) : sourceBody
  return new Blob([body], {type: mimeTypeMap[type]})
}

function parser(buffer: ArrayBufferLike) {
  try {
    ...
    if (
      ...
      throw new Error('[YmatPlayer] - init: Read Ymat file faild, invalid format!')
    }
    ...
    while (offset < totalLength) {
      ...
      if (sourceId && sourceBody) {
        sourceMap[sourceId] = getSourceBlob(type, sourceBody)
      }
    }
